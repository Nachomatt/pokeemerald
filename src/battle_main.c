...
void FaintClearSetData(void)
{
    ...
    gProtectStructs[gActiveBattler].alreadyBounced = FALSE;
...
static void TryDoEventsBeforeFirstTurn(void)
{
    ...
    if (AbilityBattleEffects(ABILITYEFFECT_INTIMIDATE1, 0, 0, 0, 0) != 0)
        return;
    if (AbilityBattleEffects(ABILITYEFFECT_TANGLEDHAIR1, 0, 0, 0, 0) != 0)
        return;
    if (AbilityBattleEffects(ABILITYEFFECT_ACIDRAIN1, 0, 0, 0, 0) != 0)
        return;
    if (AbilityBattleEffects(ABILITYEFFECT_ILLUMINATE1, 0, 0, 0, 0) != 0)
        return;
    if (AbilityBattleEffects(ABILITYEFFECT_TRACE, 0, 0, 0, 0) != 0)
        return;
    if (AbilityBattleEffects(ABILITYEFFECT_MASTERMIND, 0, 0, 0, 0) != 0)
        return;
    if (AbilityBattleEffects(ABILITYEFFECT_WEBSPINNER, 0, 0, 0, 0) != 0)
        return;
    if (AbilityBattleEffects(ABILITYEFFECT_MIGHTYROAR1, 0, 0, 0, 0) != 0)
        return;
    // if (AbilityBattleEffects(ABILITYEFFECT_TRICKSTER, 0, 0, 0, 0) != 0)
    //     return;
...
u8 IsRunningFromBattleImpossible(void)
{
    ...
    if (i != 0 && IS_BATTLER_OF_TYPE(gActiveBattler, TYPE_GRASS))
    {
        gBattleScripting.battler = i - 1;
        gLastUsedAbility = gBattleMons[i - 1].ability;
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_PREVENTS_ESCAPE;
