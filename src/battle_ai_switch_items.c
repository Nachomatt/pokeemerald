...
static bool8 FindMonThatAbsorbsOpponentsMove(void)
{
    ...
    else if (gBattleMoves[gLastLandedMoves[gActiveBattler]].type == TYPE_ROCK)
        absorbingTypeAbility = ABILITY_GEM_GOBBLER;
    else
        return FALSE;
...
static bool8 ShouldSwitch(void)
{
    ...
    {
        if (IS_BATTLER_OF_TYPE(gActiveBattler, TYPE_STEEL) || gBattleMons[gActiveBattler].ability == ABILITY_MAGNET_PULL)
            return FALSE;
    }
